...
param(
    ...
    [switch]$SupersededUpdates,
     ...
     [parameter(Mandatory=$false, HelpMessage="Use this switch to clean Software Updates that are expired, from the Deployment Package.")]
    [switch]$expiredUpdates,
...
Process {
    ...
    try {
        ...
        if ($DeploymentPackage -ne $null) {
            ...
            if (($PSBoundParameters.ContainsKey("NonDeployedUpdates")) -or ($PSBoundParameters.ContainsKey("NonRequiredUpdates")) -or ($PSBoundParameters.ContainsKey("SupersededUpdates")) -or ($PSBoundParameters.ContainsKey("expiredUpdates"))) {
            ...
            if ($PSBoundParameters["SupersededUpdates"]) {
             ...
             # Append WQL query to include Software Update instances that are superseded
            if ($PSBoundParameters["expiredUpdates"]) {
                Write-Verbose -Message "Extending the WQL query with expired updates"
                $WQLQuery = -join @($WQLQuery, "SU.isExpired = 1 OR ")
            }  
